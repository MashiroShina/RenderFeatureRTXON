...
void Outline_float(float2 UV, float OutlineThickness, float DepthSensitivity, float NormalsSensitivity, float ColorSensitivity, float4 OutlineColor, out float4 Out)
{
    ...
    float debugDepthBlur;
    float3 debugnormalBlur;
    for(float index=0;index<10;index++){
        //get uv coordinate of sample
        float2 uv = UV + float2(0, (index/9 - 0.5) * 0.025);
        float2 uv2 = UV + float2( (index/9 - 0.5) * 0.025,0);
        float2 uv3 = UV + float2((index/9 - 0.5) * 0.025, (index/9 - 0.5) * 0.025);
        float2 uv4 = UV + float2(-(index/9 - 0.5) * 0.025, (index/9 - 0.5) * 0.025);

        //add color at position to color
        debugDepthBlur += SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, uv).r;
        debugDepthBlur += SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, uv2).r;
        debugDepthBlur += SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, uv3).r;
        debugDepthBlur += SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, uv4).r;
    }
    debugDepthBlur /= 40;
       ...
       for(float index=0;index<10;index++){
        ...
        //add color at position to color
        debugnormalBlur += DecodeNormal( SAMPLE_TEXTURE2D(_CameraDepthNormalsTexture, sampler_CameraDepthNormalsTexture, uv));;
        debugnormalBlur +=  DecodeNormal( SAMPLE_TEXTURE2D(_CameraDepthNormalsTexture, sampler_CameraDepthNormalsTexture, uv));;
        debugnormalBlur +=  DecodeNormal( SAMPLE_TEXTURE2D(_CameraDepthNormalsTexture, sampler_CameraDepthNormalsTexture, uv));;
        debugnormalBlur +=  DecodeNormal( SAMPLE_TEXTURE2D(_CameraDepthNormalsTexture, sampler_CameraDepthNormalsTexture, uv));;
    }
    ...
    Out = ((1 - edge) * original) + (edge * lerp(original, OutlineColor,  OutlineColor.a));//float4(depthSamples[0]*normalSamples[0]*colorSamples[0],1)*original*15+edge*OutlineColor;//((1 - edge) * original) + (edge * lerp(original, OutlineColor,  OutlineColor.a));
}